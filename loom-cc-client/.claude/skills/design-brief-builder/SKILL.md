...
[工作流程]
    ...
    [输出阶段]
        ...
        第三步：输出文件
            保存为 docs/design/current/Design-Brief.md（目录不存在时自动创建）

        第四步：追加变更记录
            读取 docs/design/CHANGELOG.md（不存在则创建，写入标题 # Design Brief 变更记录）
            根据变更类型判断版本类型：
                MAJOR — 设计方向根本性调整（色彩体系重构、布局模式颠覆）
                MINOR — 新增设计模块、补充规范细节、调整核心视觉方向
                PATCH — 细节修正、措辞调整、参数微调
            在文件顶部插入新版本记录，格式：
                ## vX.X.X · YYYY-MM-DD
        ...
        第五步：归档旧版本
            读取 docs/design/CHANGELOG.md，获取本次写入的新版本号
            向用户说明：「本次变更将生成新版本 vX.X.X，是否确认创建？」
            用户确认后：
                - 将 docs/design/current/ 下的所有文件复制到 docs/design/versions/vX.X.X/
                - docs/design/current/ 保持最新内容不变
            用户拒绝则不创建版本，仅保留 current/ 的更新内容，不归档，撤销 CHANGELOG 中本次写入

        第六步：引导下一步
            "✅ Design Brief 已生成！

             文件：docs/design/current/Design-Brief.md（vX.X.X）

             接下来：
             - 把 docs/product/current/Product-Spec.md + Design-Brief.md 一起喂给设计工具画设计稿
