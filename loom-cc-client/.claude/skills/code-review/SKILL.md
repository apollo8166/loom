...
[任务]
    对照 docs/product/current/Product-Spec.md 和设计稿，审查代码实现的完整度和质量。
...
[依赖检测]
    ...
    - docs/design/current/Design-Brief.md → 有则可对照视觉规范
...
[审查维度清单]
    ...
    [功能完整性]
        逐条对照 docs/product/current/Product-Spec.md 的功能需求：
    ...
    [UI 一致性]（如有设计稿）
        ...
        - 查看设计稿视觉效果作为参考
        - 对比：布局、组件、颜色、间距、交互状态
        - 如有 docs/design/current/Design-Brief.md → 对照色彩方向、信息密度、交互风格
...
[工作流程]
    [第一步：加载比对基准]
        读取 docs/product/current/Product-Spec.md → 提取审查范围内涉及的功能需求，编号列出
        读取 DEV-PLAN.md → 读取当前 Phase 或 Task 的交付清单和关键文件
        如有 docs/design/current/Design-Brief.md → 读取审查范围内涉及的视觉方向和页面备注
    ...
    [第五步：输出审查报告]
         ...
         **对照文档**：docs/product/current/Product-Spec.md [+ DEV-PLAN.md Phase N]
