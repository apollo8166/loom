...
export async function POST(req: NextRequest) {
  let body: { title?: string; model?: string; workspacePath?: string; attachedFolderPaths?: string[]; useWorktree?: boolean; runtimeTarget?: 'local' } = {}
  try {
    body = await req.json()
  ...
  db.prepare(
    `INSERT INTO sessions (id, project_id, title, model, runtime_session_id, workspace_path, attached_folder_paths, use_worktree, runtime_target)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`
  ).run(
    sessionId,
