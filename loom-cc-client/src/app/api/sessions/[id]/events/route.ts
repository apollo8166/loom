...
export async function GET(
  ...
  const stream = new ReadableStream<Uint8Array>({
    start(controller) {
      let closed = false
      let cursor = hasLastEventId ? lastEventId : getRuntimeEventsCursor('session', id)
      ...
      for (const event of readActiveSessionEvents(id)) send(event, { replay: !hasLastEventId })
      const unsubscribe = subscribeSessionEvents(id, send)
      const heartbeat = setInterval(() => send({ type: 'ping' }), 25_000)
      const poll = setInterval(() => {
        for (const event of readRuntimeEvents('session', id, cursor)) send(event)
      }, 500)
