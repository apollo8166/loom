---
name: dev-planner
description: 当 docs/product/current/Product-Spec.md 已完成、需要规划怎么分阶段开发时使用。也在 Spec 变更后用于更新已有开发计划。输出 DEV-PLAN.md。
---

[任务]
    **生成模式**：读取 docs/product/current/Product-Spec.md（和 Design-Brief.md，如有），分析功能依赖关系，WebSearch 验证技术选型，输出分阶段开发计划 DEV-PLAN.md。
...
[依赖检测]
    ...
    可选（降级模式）：
    - docs/design/current/Design-Brief.md → 缺失则标记"无设计规范模式"，视觉相关细节标注 [待 Design Brief 补充]
...
[工作流程（生成模式）]
    [加载阶段]
        ...
        第二步：加载 Product Spec
            读取 docs/product/current/Product-Spec.md
            提取：产品类型、核心功能列表、辅助功能列表、AI 能力需求、技术方向、UI 布局结构、数据存储方式
            检查 docs/product/current/Product-Spec.md 中是否包含 [待补充] 标记。如有，列出涉及的条目并提示用户先补充或确认可以跳过

        第三步：加载 Design Brief（如有）
            读取 docs/design/current/Design-Brief.md
    ...
    [输出阶段]
        ...
        第四步：输出文件
            在文件顶部写入版本注释：<!-- version: X.X.X --> 和 <!-- updated: YYYY-MM-DD — 变更摘要 -->
            保存为 docs/dev/current/DEV-PLAN.md

        第五步：追加变更记录
            读取 docs/dev/CHANGELOG.md（不存在则创建，写入标题 # DEV-PLAN 变更记录）
            根据变更类型判断版本类型：
                MAJOR — Phase 结构根本性重排、技术栈重大调整、Phase 数量大幅变化
                MINOR — 新增 Phase、拆分/合并 Phase、添加新功能到已有 Phase
                PATCH — 文件路径修正、验收标准补充、进度标记更新（✅/⏸️/🟨）
            在文件顶部插入新版本记录，格式：
                ## vX.X.X · YYYY-MM-DD
                **类型**：MAJOR/MINOR/PATCH — 一句话描述
                **变更内容**：
                ### 新增（如有）
                - ...
                ### 修改（如有）
        ...
        第六步：归档旧版本
            读取 docs/dev/CHANGELOG.md，获取本次写入的新版本号
            向用户说明：「本次变更将生成新版本 vX.X.X，是否确认创建？」
            用户确认后：
                - 将 docs/dev/current/ 下的所有文件复制到 docs/dev/versions/vX.X.X/
                - docs/dev/current/ 保持最新内容不变
            用户拒绝则不创建版本，仅保留 current/ 的更新内容，不归档，撤销 CHANGELOG 中本次写入

        第七步：引导下一步
            "✅ DEV-PLAN.md 已生成！

             文件：DEV-PLAN.md（vX.X.X）
...
[工作流程（迭代模式）]
    ...
    [变更分析阶段]
        第一步：加载现有文件
            读取现有 DEV-PLAN.md
            读取更新后的 docs/product/current/Product-Spec.md
            如有 Product-Spec-CHANGELOG.md → 读取最近的变更记录，快速定位变更范围
            如有 docs/design/current/Design-Brief.md → 读取，检查视觉方向是否也有变更
    ...
    [更新阶段]
        ...
        第三步：保存文件
            更新文件顶部的版本注释：<!-- version: X.X.X --> 和 <!-- updated: YYYY-MM-DD — 变更摘要 -->
            保存更新后的 DEV-PLAN.md

        第四步：追加变更记录
            按照 [工作流程（生成模式）] 第五步的相同格式和规则，写入 docs/dev/CHANGELOG.md

        第五步：归档旧版本
            按照 [工作流程（生成模式）] 第六步的相同流程，向用户确认后归档到 docs/dev/versions/vX.X.X/
