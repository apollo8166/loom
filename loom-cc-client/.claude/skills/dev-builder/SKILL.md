...
[依赖检测]
    ...
    必需：
    - docs/product/current/Product-Spec.md → 缺失则提示先调用 /product-spec-builder
    - DEV-PLAN.md → 缺失则提示先调用 /dev-planner
    - DEV-PLAN 技术栈表中列出的所有系统工具和运行时环境

    可选：
    - docs/design/current/Design-Brief.md → 缺失则标记"无设计规范模式"
    - 设计工具 MCP → 缺失则标记"无设计稿模式"
    - Gitee Access Token → 有则可通过 API 创建 Gitee 私有仓库并 push
...
[开发规则清单]
    ...
    [项目结构规范]
        ...
        ```
        project/
        ├── docs/product/current/Product-Spec.md         # 根目录，不进 git
    ...
    [Git 工作流]
        ...
        Push 策略：
        - **只在用户明确要求时才 push**（用户说"push"、"推送"、"上传"等明确指令）
        - 不要在每次 commit 后自动 push，commit 和 push 是两个独立动作
        - push 前确认当前分支正确
        - 如远程仓库未设置 → 提醒用户先配置
...
[开发策略]
    ...
    **每个功能开发后**：
    ...
    - 如无 Design-Brief → 以 docs/product/current/Product-Spec.md 文字描述为参照
...
[工作流程（初始化模式）]
    ...
    [启动阶段]
        ...
        第二步：加载文档
            读取 docs/product/current/Product-Spec.md → 提取产品概述、核心功能
            读取 DEV-PLAN.md → 提取技术栈表、Phase 1 内容、数据库表（如有）
            如有 docs/design/current/Design-Brief.md → 读取色彩方向、信息密度（配置 Tailwind 主题）
    ...
    [项目搭建阶段]
        ...
        Git 准备（使用 Gitee，国内访问稳定）：
        1. 根目录 git init + 创建 .gitignore（排除规划文档、设计资源、环境变量、构建产物）
        2. 通过 Gitee Open API 创建私有仓库（需用户提供 Access Token）
           - 个人仓库：POST https://gitee.com/api/v5/user/repos
           - curl 示例：curl -X POST "https://gitee.com/api/v5/user/repos" -H "Content-Type: application/json" -d '{"access_token":"<token>","name":"<repo>","private":true}'
        3. git remote add origin https://gitee.com/<username>/<repo>.git
        4. 首次 commit + push
...
[工作流程（持续开发模式）]
    ...
    [加载阶段]
        ...
        第二步：加载文档和代码状态
            ...
            读取 docs/product/current/Product-Spec.md → 作为功能参照
            如有 docs/design/current/Design-Brief.md → 读取视觉方向
            如有设计工具 MCP → 准备读取
            扫描已有代码结构 → 了解当前项目状态
    ...
    [Phase 执行流程]
        第一步：Plan + TaskList
            ...
            2. 如有设计工具 MCP 已连接，查看该 Phase 涉及的页面，读取精确数值。如无设计工具，以 docs/design/current/Design-Brief.md 或 docs/product/current/Product-Spec.md 为参照
            3. 探索现有代码，理解当前结构
        ...
        第二步：逐个 Task 实现 + 单 Task Review 循环
            ...
            2. 读取 docs/product/current/Product-Spec.md 中该 Task 涉及的功能描述
            ...
            8. 对照 docs/product/current/Product-Spec.md 确认功能行为符合描述
            9. 派发 code-reviewer 执行两阶段审查。code-reviewer 同样对照 docs/product/current/Product-Spec.md、docs/design/current/Design-Brief.md、DEV-PLAN.md 和设计稿审查
...
[初始化]
    ...
    - 无 docs/product/current/Product-Spec.md → 提示先调用 /product-spec-builder
