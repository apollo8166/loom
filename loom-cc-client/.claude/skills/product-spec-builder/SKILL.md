...
[依赖检测]
    ...
    - 0-1 模式：无前置文件要求
    - 迭代模式：docs/product/current/Product-Spec.md 必须存在

[文档分层规则]
    从现在开始，Product Spec 采用“主文档 + 模块文档”结构，禁止继续把所有功能细节塞回主 PRD。

    **主文档路径**：
    - `docs/product/current/Product-Spec.md`

    **模块文档目录**：
    - `docs/product/current/modules/`
    ...
    **新增功能归属判断规则**：
    1. 影响整个产品的信息架构、定位、跨模块共享机制 → 改主文档
    2. 只属于某个具体模块的能力、页面、流程、交互 → 改对应模块文档
    3. 如果是全新一级模块 → 先更新主文档的模块地图，再创建对应模块文档
    4. 如果用户新增的是现有模块功能 → 优先更新对应模块文档，不要把细节重新堆回主文档

    **模块文档命名规则**：
    - 统一使用英文 slug 文件名
    - 示例：`chat.md`、`ai-classroom.md`、`practice-wrongbook.md`
...
[启动检查]
    ...
    第二步：扫描项目目录，按优先级查找产品需求文档
        优先级1（精确匹配）：docs/product/current/Product-Spec.md
        优先级2（扩大匹配）：docs/product/current/ 下的 *spec*.md、*prd*.md、*PRD*.md、*需求*.md、*product*.md
    ...
    第三步：检查模块文档目录
        - 若 `docs/product/current/modules/` 存在 → 进入模块化 PRD 模式
        - 若不存在 → 按传统单文档模式处理，但一旦发生结构重构，应升级为模块化 PRD
...
[工作流程（0-1模式）]
    ...
    [文档生成阶段]
        ...
        第五步：输出文件
            将 Product Spec 保存为 docs/product/current/Product-Spec.md
            同时归档到 docs/product/versions/v1.0.0/Product-Spec.md（首次生成固定为 v1.0.0）
            在 docs/product/CHANGELOG.md 追加本次版本记录
...
[工作流程（迭代模式）]
    ...
    **模块化优先原则**：
    - 若项目已存在 `docs/product/current/modules/`，默认进入模块化 PRD 处理方式
    - 新增功能时，先判断它属于主文档还是某个模块文档
    - 不允许图省事把模块细节重新写回主 Product-Spec.md
    ...
    [追问完善阶段]
        ...
        第三步：冲突检测
            ...
            如为模块化 PRD，同时加载相关模块文档
            检查新需求是否与现有内容冲突
            发现冲突 → 直接指出冲突点 + 给解决方案 + 让用户选

        **停止追问的标准**：
        - 能够直接动手改对应文档，不需要再猜或假设
        - 改完之后用户不会说"不是这个意思"
    ...
    [文档更新阶段]
        ...
        第一步：理解现有文档结构
            加载现有主文档
            如果存在模块目录，同时识别相关模块文档结构
            后续修改基于现有结构，不强行套用模板

        第二步：按归属修改文档
            - 产品级变更：修改 `docs/product/current/Product-Spec.md`
            - 现有模块变更：优先修改对应模块文档
            - 新一级模块：先更新主文档模块地图，再创建模块文档
        ...
        第四步：自动追加变更记录
            变更记录统一写入 docs/product/CHANGELOG.md
            如果文件不存在，创建一个
            记录 Product Spec 迭代变更时，加载 templates/changelog-template.md 获取完整的变更记录格式和示例
            根据对话内容自动生成变更描述

        第五步：归档旧版本
            读取 docs/product/CHANGELOG.md，获取当前最新版本号
            根据变更类型判断新版本号（MAJOR/MINOR/PATCH）
            向用户说明："本次变更将生成新版本 vX.X.X，是否确认创建？"
            用户确认后，才执行以下操作：
            - 将 docs/product/current/ 下的所有文件复制到 docs/product/versions/vX.X.X/
            - 用新内容覆盖 docs/product/current/ 下的对应文件
            用户拒绝则不创建版本，仅更新 current/ 内容，不归档，不写 CHANGELOG
