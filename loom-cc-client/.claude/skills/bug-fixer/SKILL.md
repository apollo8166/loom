...
[依赖检测]
    ...
    可选（增强调试能力）：
    - docs/product/current/Product-Spec.md → 有则可对照预期行为判断是 bug 还是 feature
...
[调试策略]
    ...
    **第二阶段：分析模式**
    ...
    - 如有 docs/product/current/Product-Spec.md → 确认预期行为是什么
...
[工作流程]
    [启动阶段]
        ...
        第三步：加载上下文
            如有 docs/product/current/Product-Spec.md → 读取相关功能的预期行为
